...
def run(httpClient, httpResponse):
    ...
    content = {}
    print("Memory at start of /run:", gc.mem_free())
    if stop:
        ...
        print("Memory before menu.stop:", gc.mem_free())
        menu.stop()
        dprint("Menu stopped")
        print("Memory after menu.stop:", gc.mem_free())
        import bot
        bot.reset_terminal()
        print("Memory after terminal reset:", gc.mem_free())
        gc.collect()
        print("Memory after gc.collect:", gc.mem_free())
        try:
            import weditor.pmanager
            print("About to call restart_process with:", name)
            print("Memory before restart_process:", gc.mem_free())
            weditor.pmanager.restart_process(name)
            print("restart_process completed")
            print("Memory after restart_process:", gc.mem_free())
            content = {"started": True}
        except Exception as e:
            print("Error in restart_process:", str(e))
            content = {"error": str(e)}

    print("Memory at end of /run:", gc.mem_free())
    _respond(httpResponse, content)
